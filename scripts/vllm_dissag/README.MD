...
`DeepSeek-V4-Flash-FP8` / `DeepSeek-V4-Pro-FP8` keep two per-model images (same
naming as `vllm_disagg_inference.glmv5.1.ubuntu.amd.Dockerfile`). Pins live in
each Dockerfile header, not the filename:

- `--image v0280` — `docker/vllm_disagg_inference.dsv4.v0280.ubuntu.amd.Dockerfile`
- `--image v0290` — `docker/vllm_disagg_inference.dsv4.v0290.ubuntu.amd.Dockerfile`

Do not docker-build on login or WSL. Submit with `./run_wideep_bench.sh` and
**always pass `--image`**. Do not mix v0290 scores with v0280. Hub
`vllm/vllm-openai-rocm:v0.29.0` is still ROCm 7.2.3; do not install UFB
`+rocm10.1.0a` AITER wheels on it.
