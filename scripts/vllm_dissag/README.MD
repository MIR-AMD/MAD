...
| `DeepSeek-V4-Flash-FP8` | MoE | 3 (moriio wideEP) | own image (`dsv4.v0290` Dockerfile); MoRI-EP only; submit via `run_wideep_bench.sh` |
| `DeepSeek-V4-Pro-FP8` | MoE | 3 (moriio wideEP) | own image (`dsv4.v0290` Dockerfile); MoRI-EP only; submit via `run_wideep_bench.sh` |
...
`DeepSeek-V4-Flash-FP8` / `DeepSeek-V4-Pro-FP8` need their own image for the same
reason GLM does — their vLLM/AITER/MoRI pins differ from the shared Dockerfile —
and follow the same per-model naming:

```bash
docker build -t <your-registry>/vllm-disagg:dsv4-v0290 \
  -f docker/vllm_disagg_inference.dsv4.v0290.ubuntu.amd.Dockerfile .
```

Exact pins live in that Dockerfile's header, not in the filename: vLLM release
v0.29.0, AITER and MoRI from source, `flydsl==0.3.2`, and `vllm-router` baked at
`/usr/local/bin/vllm-router` (so leave `ROUTER_BOOT_INSTALL` unset). Like the GLM
image it defaults to `WITH_NIXL=0` — DSV4 runs MoRI-EP only. The Hub base is still
ROCm 7.2.3, so do **not** install UFB `+rocm10.1.0a` AITER wheels on it.

Do not build on a login node or under WSL; build on a compute host and push to a
registry the cluster can pull from.

### Launching DSV4 jobs

DSV4 submits through `./run_wideep_bench.sh <bench> <model> <topology>`, which
derives every dependent knob from those three arguments. The image above is the
default for both DSV4 models, so `--image` is only needed to override it:

```bash
cd MAD/scripts/vllm_dissag

# curl + NIAH ladder + concurrency sweep in one allocation (the usual first run)
./run_wideep_bench.sh validate dsv4fls ep16

# a single bench
./run_wideep_bench.sh niah  dsv4pro 2p2d
./run_wideep_bench.sh smoke dsv4fls 1p1d

# print the sbatch line without submitting
./run_wideep_bench.sh validate dsv4fls ep32 --dry-run
```

`dsv4fls` / `dsv4pro` are shorthand for the two `MODEL_NAME` values; topology is
`xPyD` (`2p2d`) or an EP width (`ep16`, `ep32`). Useful flags: `--time HH:MM:SS`,
`--exclude <nodelist>`, `--after JOBID` to queue behind a running job, and
`QOS=low` where the partition requires it.

Prompt defaults are handled per model and should not be set by hand — the Flash
NIAH branch primes the answer stem (`NIAH_LIST_PRIME=1.`) and the Pro branch runs
unprimed. The MoRI dispatch trim is on by default; `MORI_TRIM_DISPATCH=0` opts out.
